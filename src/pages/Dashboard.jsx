...
function DashboardCard({
  ...
  return (
    <div
      className={`dashboard-card ${className} ${gradient ? "gradient-border" : ""}`}
    >
...
function Dashboard() {
  ...
  const currentLearningInfo = useMemo(() => {
    if (isStatsLoading)
      return { title: "Loading...", progress: 0, topic: "Please wait" };
    if (!learningStats || !learningStats.current_roadmap) {
      return {
        title: "Start a learning track",
        progress: 0,
        topic: "Visit the Learn page",
      };
    }
  ...
  const latestProject = useMemo(() => {
    if (projects.length === 0) {
      return {
        ...
        stack: "Start your first project",
        progress: 0,
      };
    }
    return projects[0];
  ...
  const latestReply =
    history.length > 0 && history[0].reply
      ? history[0].reply
      : "Ask me what to learn next.";
  const previewText =
    latestReply.length > 100
      ? latestReply.substring(0, 100) + "..."
      : latestReply;
    ...
    <div className="dashboard">
      ...
      <div className="cards-grid">
          ...
          <div className="course-preview">
            ...
            <div className="course-info">
              ...
              <span className="progress-text">
                {Math.round(currentLearningInfo.progress)}% complete
              </span>
            </div>
          </div>
          <button
            ...
            onClick={() =>
              currentLearningInfo.id
                ? navigate(`/learn/roadmap/${currentLearningInfo.id}`)
                : navigate("/learn")
            }
          >
            {currentLearningInfo.id ? "Continue" : "Explore Roadmaps"}{" "}
            <ArrowRight size={16} />
          </button>
        </DashboardCard>
        ...
        >
          <div className="mentor-preview">
            ...
            <div className="mentor-info">
              ...
              <span className="timestamp">
                {history.length > 0 ? "Recently" : "Not asked yet"}
              </span>
            </div>
          </div>
          <button
          ...
          >
            Ask AI Mentor <ArrowRight size={16} />
          </button>
        </DashboardCard>
          ...
          <div className="project-info">
            ...
            <div className="project-stats">
              ...
              <div>
                ...
                <span>{Math.round(latestProject.progress)}% done</span>
              </div>
            </div>
          </div>
          <button className="action-btn" onClick={() => navigate("/projects")}>
            {projects.length > 0 ? "Open Project" : "Start First Project"}{" "}
            <ArrowRight size={16} />
          </button>
        </DashboardCard>

        <DashboardCard
          title="Today's Productivity Plan"
          icon={CheckCircle2}
        ...
        >
          ...
          <div className="plan-progress">
            <span>
              {productivityStats.completedTasks} of{" "}
              {productivityStats.totalTasks} completed
            </span>
